...
/*   Updated: 2020/09/16 17:23:47 by jfleury          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
import { getUserById, addUser, deleteUser, activateUser, getUserByEmail } from '../model/userRepositories';
import { addUserValidation } from '../services/addUserValidation';
import { generateTokenForUser, jwtVerify, JWT_SIGN_SECRET } from '../services/jwt';
...
export async function addUserController(req: Request, res: Response) {
	...
	if (userValidation) {
		...
		if (user) {
			...
			mailer(user, `http://localhost:3001/user/activateUser?activationKey=${user.activationKey}&id=${user.id}`);
			return;
		}
	}
